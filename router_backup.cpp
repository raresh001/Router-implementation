...
private:
    ...
    void send_over_IPv4(char *buf, size_t len);

    void manage_IPv4_package(char *buf, size_t len, int interface);
    void manage_ICMP_package(char *buf, size_t len, int interface);
...
void Router::run() {
    ...
    while (1) {
        ...
        switch (ntohs(eth_hdr->ether_type)) {
            case ETHERTYPE_IP:
                manage_IPv4_package(buf, len, interface);
                break;
            case ETHERTYPE_ARP:
...
void Router::send_over_IPv4(char *buf, size_t len) {
    ether_header *eth_hdr = (ether_header *) buf;
...
void Router::manage_IPv4_package(char *buf, size_t len, int interface) {
    ether_header *eth_hdr = (ether_header *) buf;
    iphdr *ip_hdr = (iphdr *)(eth_hdr + 1);
    ...
    if (ip_hdr->protocol == IPPROTO_ICMP) {
        // treat ICMP request separately
        manage_ICMP_package(buf, len, interface);
        return;
    }

    send_over_IPv4(buf, len);
}

void Router::manage_ICMP_package(char *buf, size_t len, int interface) {
    ether_header *eth_hdr = (ether_header *)buf;
    iphdr *ip_hdr = (iphdr *)(eth_hdr + 1);
    icmphdr *icmp_hdr = (icmphdr *)(ip_hdr + 1);

    // get the ip address in this interface
    char *ip_str = get_interface_ip(interface);
    uint32_t ip;
    inet_pton(AF_INET, ip_str, &ip);

    // see if the router is the destination of this package and the message was Echo request
    if (ip == ip_hdr->daddr && icmp_hdr->type == 8 && icmp_hdr->code == 0) {
        // send echo reply
        icmp_hdr->type = 0;
        icmp_hdr->code = 0;
    } else {
